...
<main class="content" id="main-content">
      ...
      <div class="greet">
        ...
        <p>
          <?php if ($totalApplications > 0): ?>
            ...
            <?php if (!empty($savedJobs) && $savedJobs > 0): ?> and <strong><?= $savedJobs ?></strong> saved <?= $savedJobs == 1 ? 'job' : 'jobs' ?><?php endif; ?> waiting for your action.
          <?php else: ?>
    ...
    <section class="ai-hero" aria-labelledby="ai-title">
      <div class="ai-hero-grid">
        <div>
          ...
          <p class="ai-sub">
            <?php $pending = array_filter($applications ?? [], fn($a) => strtolower($a->status ?? '') === 'pending'); $pendingCount = count($pending); ?>
            <?php if ($pendingCount > 0): ?>You have <strong><?= $pendingCount ?></strong> pending <?= $pendingCount == 1 ? 'application' : 'applications' ?> awaiting employer response<?= !empty($savedJobs) && $savedJobs > 0 ? ' and <strong>' . $savedJobs . '</strong> saved ' . ($savedJobs == 1 ? 'job' : 'jobs') . ' to revisit' : '' ?>.
            <?php else: ?>Your AI engine is ready. Browse jobs, apply, and track every employer response in real time.<?php endif; ?>
          </p>
        </div>
    ...
    <section class="stats" aria-label="Your activity" style="display:grid">
      <div class="stat"><div class="stat-top"><span class="stat-ic"><svg aria-hidden="true"><use href="#i-doc"/></svg></span></div><div class="stat-num"><?= $totalApplications ?></div><div class="stat-lbl">Total Applications</div></div>
      <div class="stat" style="--st-bar:var(--accent);--st-icbg:var(--accent-light);--st-ic:var(--accent-dark)"><div class="stat-top"><span class="stat-ic"><svg aria-hidden="true"><use href="#i-bookmark"/></svg></span></div><div class="stat-num"><?= $savedJobs ?></div><div class="stat-lbl">Saved Jobs</div></div>
      <div class="stat" style="--st-bar:var(--brand-dark)"><div class="stat-top"><span class="stat-ic"><svg aria-hidden="true"><use href="#i-eye"/></svg></span></div><div class="stat-num"><?= $jobsViewed ?></div><div class="stat-lbl">Jobs Viewed</div></div>
