    ...
    <div class="detail-layout">
      ...
      <div class="detail-main">
        ...
        <div class="detail-card">
          ...
          <!-- Read-more button — hidden on desktop, shown on mobile only -->
        ...
        <?php if (($job->application_method ?? 'form') === 'form'): ?>
          <div id="apply-form-section" class="detail-card" style="display:none">
            <div class="apply-form">
              <h2 class="detail-section-title">Apply for this Job</h2>
              <p class="text-muted mb-4">Complete your application to be considered for this position. All fields marked with * are required.</p>
              ...
              <form id="inlineApplyForm" novalidate>
                ...
                <div class="form-row">
                  <div class="form-group mb-3">
                    <label class="form-label" for="apply_name">Full Name <span class="text-danger">*</span></label>
                    <input type="text" id="apply_name" name="full_name" required class="form-control" placeholder="John Doe" autocomplete="name">
                    <div class="form-error"></div>
                  </div>
                  <div class="form-group mb-3">
                    <label class="form-label" for="apply_email">Email Address <span class="text-danger">*</span></label>
                    <input type="email" id="apply_email" name="email" required class="form-control" placeholder="john@example.com" autocomplete="email">
                    <div class="form-error"></div>
                  </div>
                </div>
                
                <div class="form-group mb-3">
                  <label class="form-label" for="apply_phone">Phone Number <span class="text-danger">*</span></label>
                  <input type="tel" id="apply_phone" name="phone" required class="form-control" placeholder="+234 800 000 0000" autocomplete="tel">
                  <div class="form-error"></div>
                </div>
                
                <div class="form-group mb-3">
                  <label class="form-label" for="apply_resume">Resume/CV <span class="text-danger">*</span></label>
                  <input type="file" id="apply_resume" name="resume" required accept=".pdf,.doc,.docx,.txt" class="form-control">
                  <div class="form-text">Upload your CV in PDF, DOC, or DOCX format (max 5MB)</div>
                  <div class="form-error"></div>
                </div>
                
                <div class="form-group mb-3">
                  <label class="form-label" for="apply_cover">Cover Letter (Optional)</label>
                  <textarea id="apply_cover" name="cover_letter" rows="5" class="form-control" placeholder="Tell us about yourself..."></textarea>
                </div>
                
                <div class="form-group mb-3">
                  <label class="form-label" for="apply_referral">How did you hear about this job? <span class="text-danger">*</span></label>
                  <select id="apply_referral" name="referral" required class="form-select">
                ...
                <div class="form-actions">
                  <button type="button" id="cancelApply" class="btn btn-outline">Cancel</button>
        ...
        <div class="apply-card apply-card--featured">
          ...
          <?php if ($job->is_featured || ($job->featured_until && strtotime($job->featured_until) > time())): ?>
            <div style="display:flex;align-items:center;gap:7px;font-size:.78rem;font-weight:700;color:#dc2626;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:8px 12px;margin-bottom:12px">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 2 4.1 13H11l-2 9 10.9-11H13l2-9z"/></svg> Urgently hiring — apply before deadline
            </div>
          <?php endif; ?>
          ...
          <?php if ($appLimit > 0): ?>
            ...
            <div style="display:flex;align-items:center;gap:7px;font-size:.78rem;font-weight:700;color:#7c2d12;background:#fff7ed;border:1px solid #ffedd5;border-radius:8px;padding:8px 12px;margin-bottom:12px">
              ...
              <?= $spotsRemaining ?> spots remaining — closes at <?= $appLimit ?> applications
            </div>
          <?php endif; ?>
          ...
          <div class="apply-actions">
            ...
            <?php if (auth()->loggedIn() || !$requiresAuth): ?>
              <?php if ($job->application_method === 'form'): ?>
                <button class="btn btn-primary btn-lg apply-external" style="width:100%;justify-content:center" onclick="toggleApplyForm()">
          ...
          <?php if (!empty($questions)): ?>
            <div class="ats-notice" style="display:flex;align-items:start;gap:8px;background:#f0fdf4;border:1px solid #bbf7d0;padding:10px;border-radius:8px;font-size:.8rem;color:#166534">
              ...
              <span>This employer uses <strong>pre-screening questions</strong>. You'll be asked a few short questions when you apply — this helps your application stand out.</span>
            </div>
          <?php endif; ?>
        </div>

        <!-- JOB OVERVIEW — structured facts panel (matches live page) -->
        ...
        <div class="company-card">
          <div class="company-card-head">
            ...
            <div>
              ...
              <div class="company-card-meta"><?= esc($job->industry_name ?? 'General Business') ?> · <?= $employer_job_count ?> open roles</div>
            </div>
          </div>
            ...
            <div class="job-salary-row" style="margin-top:auto">
              <span class="job-salary" style="font-weight:700;color:var(--brand);font-size:.88rem">
                <?php if ($related->salary_type === 'range'): ?>
                  ₦<?= number_format($related->salary) ?> - ₦<?= number_format($related->salary_max) ?>
                <?php elseif ($related->salary_type === 'fixed'): ?>
                  ₦<?= number_format($related->salary) ?>
                <?php else: ?>
                  Negotiable
                <?php endif; ?>
              </span>
